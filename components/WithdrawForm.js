  ...
  return (
    <form onSubmit={submit} className="border border-line rounded-2xl p-5">
      ...
      <p className="text-xs text-ink/50 mb-3">
        Minimum <Naira amount="5,000" /> · Available: <Naira amount={balanceKobo / 100} />
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
        <input required type="number" placeholder="Amount (₦)" value={form.amount}
          onChange={(e) => setForm({ ...form, amount: e.target.value })}
          className="border border-line rounded-lg px-3 py-2.5 text-base sm:text-sm focus-ring" />
      ...
      <button disabled={loading} className="w-full sm:w-auto bg-ink text-paper px-5 py-3 sm:py-2.5 rounded-full text-sm font-medium hover:bg-biro transition-colors">
