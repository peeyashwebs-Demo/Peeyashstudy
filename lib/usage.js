...
export const LIMITS = { uploads: 3, quizzes: 2, chat: 5, worked_example: 2, explain_back: 5 }; // uploads+quizzes+worked_example per month, chat+explain_back per day
...
export async function checkAndCount(admin, userId, kind, isPremium) {
  ...
  const period = (kind === "chat" || kind === "explain_back") ? dayKey() : monthKey();
